"""refer from https://github.com/Lyken17/pytorch-memonger/blob/master/memonger/checkpoint.py"""
...
class CheckpointFunction(torch.autograd.Function):
    ...
    def forward(ctx, run_function, preserve_rng_state, *args):
        ...
        # ctx.save_for_backward(*args, **kwargs) 
    ...
    def backward(ctx, *args):
        ...
        with torch.random.fork_rng(devices=rng_devices, enabled=ctx.preserve_rng_state):
            ...
            with torch.enable_grad():
                # outputs = ctx.run_function(*detached_inputs, **kwargs)
                outputs = ctx.run_function(*detached_inputs)
        if isinstance(outputs, torch.Tensor):
            outputs = (outputs,)
        torch.autograd.backward(outputs, args) # 求导
...
def checkpoint(function, *args, **kwargs):
    ...
    return CheckpointFunction.apply(function, preserve, *args, **kwargs)
